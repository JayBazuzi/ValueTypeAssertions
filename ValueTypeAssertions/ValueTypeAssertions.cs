	...
	public static class ValueTypeAssertions
	{
		public static void HasValueEquality<T>(T item, T equalItem)
		{
			...
			if (item is IEquatable<T>)
			{
				...
				equatable.Invoking(_ => _.Equals(default(T))).Should().NotThrow<NullReferenceException>("IEquatable<>.Equals(null)");
			...
			item.Invoking(_ => _.Equals(null)).Should().NotThrow<NullReferenceException>("Equals(object null)");
			item.Equals(null).Should().BeFalse("Equals(object null)");
			item.Invoking(_ => _.Equals(new object())).Should().NotThrow<InvalidCastException>("compare to other type");
